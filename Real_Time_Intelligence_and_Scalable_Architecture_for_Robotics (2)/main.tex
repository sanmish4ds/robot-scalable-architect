...
\end{table}
% --- Pilot reporting figures (use measured plots; repository defaults may be illustrative/synthetic) ---
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{fig_pilot_latency_cdf}
  \caption{Schematic reporting format: end-to-end latency CDF
  for the instrumented perception path, with one empirical
  step-CDF curve per condition. Each curve should pool all
  samples collected under that condition. This schematic
  illustrates the required format; measured curves from a
  physical deployment would replace it.}
...
\begin{figure}[htbp]
  ...
  \caption{Schematic reporting format: end-to-end latency summary
  by condition. Bars show p50, p95, p99, and max derived from
  measured samples. The start/stop timestamps used to compute
  camera$\rightarrow$detection latency must be specified in the
  experiment setup text. This schematic illustrates the required
  format; measured bars from a physical deployment would replace it.}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Schematic reporting format: monitoring overhead per
  component (CPU share~\%, RSS memory in MB, and added latency
  $\Delta$ms as defined in Table~\ref{tab:pilot_overhead}).
  This schematic illustrates the required format; measured values
  from the instrumented prototype would replace it.}
...
\begin{figure}[htbp]
  ...
  \caption{Pilot experiment: supervisor-selected active tier (ordinal) vs. wall-clock elapsed time. This plot uses an illustrative compressed timeline with nominal [0,4)\,s, induced load [4,8)\,s, and recovery [8,11.5]\,s; Tier~1 (full) is plotted at the top, Tier~2 (reduced) in the middle, and Tier~3 (fallback) at the bottom.}
  \label{fig:tiertrace}
\end{figure}
...
reflects both service time and queueing delay under contention. The three tiers are
modeled as different service-time distributions (Tier~1 highest quality / heaviest tail,
Tier~2 compressed, Tier~3 emergency). A stress phase increases tail latency and injects
additional ``blocking'' time to emulate CPU contention and competing callbacks (consistent
with the executor/tail-latency effects reported in ROS~2 measurements \cite{kronauer2021,casini2019}).
