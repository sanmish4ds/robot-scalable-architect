...
% =============================================================================
% Double-blind build: compile main_blind.tex (defines \blindmodetrue before \input).
\ifdefined\blindmodetrue
  \def\blindmode{true}
\else
  \def\blindmode{false}
\fi
...
% =============================================================================
\input{experiment_setup_macros}

\title{Real-Time Intelligence and Scalable Architecture for Robotics}

\ifthenelse{\equal{\blindmode}{true}}{%
  \author{Anonymous Author(s)}%
  \institute{Affiliations withheld for double-blind peer review}%
...
\begin{abstract}
Deploying learned perception in safety-critical robotic systems creates a fundamental tension: machine learning optimizes average-case accuracy, while physical systems demand worst-case timing guarantees. This paper addresses how these two worldviews can be reconciled through explicit, runtime-monitored contracts that couple adaptive inference quality to safety-critical execution envelopes.

This paper is a \textbf{survey/tutorial} with a \textbf{reference-specification} contribution: it treats real-time intelligence and scalable architecture as a single end-to-end systems problem spanning deadlines, execution-time variability, middleware quality of service (QoS), and the edge-cloud division of computation.
...
\keywords{adaptive intelligent systems \and uncertainty-aware control \and emergent fleet behavior \and runtime contract monitoring \and real-time robotics \and machine learning inference deployment \and end-to-end latency monitoring \and distributed robotic systems \and edge robotics (edge computing) \and digital twins for robotics \and scalability \and ROS~2 (Robot Operating System~2) \and robotics middleware \and DDS/QoS}
\end{abstract}
...
\textit{Numeric examples in tables and worked calculations (except the discrete-event results in Section~\ref{subsec:sim_results}) are pedagogical illustrations of reporting format and contract semantics; deployed systems must be validated using the measurement protocol in Section~\ref{sec:eval}.}
...
\paragraph{Worked comparison (Option A): ROSRV vs.\ TIC on one AMR path.}
To make the comparison concrete, consider the warehouse AMR perception path used throughout this paper:
\emph{camera frame publish \(\rightarrow\) detections publish} with a 30\,Hz period and a 25\,ms deadline.
In ROSRV, the dominant artifact is a \emph{message-level property} over the ROS communication graph (e.g., a temporal logic assertion that a detection message should follow each camera message within a bounded time window, and that certain fields satisfy application invariants). ROSRV therefore naturally expresses \emph{ordering and safety properties} over topic streams, and can be used to raise alarms or trigger a property-driven response.

TIC adds three elements that are not first-class in ROSRV-style specifications: (i) a \emph{latency envelope} expressed as percentiles (p50/p95/p99/max) under declared stressors, not only a single ``within \(\Delta\)'' bound; (ii) \emph{learning evidence} (calibrated uncertainty and drift metrics) that gates trust in detections; and (iii) an \emph{executable fallback state machine} that binds monitoring outcomes to a tiered safety envelope (e.g., Tier~3 implies a safe-stop-compatible speed/stop constraint). The practical distinction is that ROSRV can state and monitor properties, while TIC couples timing evidence, learning evidence, and degraded-mode behavior into a single auditable contract document plus a runtime status stream.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}L{0.22\linewidth}L{0.14\linewidth}L{0.14\linewidth}L{0.14\linewidth}L{0.14\linewidth}L{0.17\linewidth}@{}}
    ...
    \textcolor{white}{\textbf{TIC (present work)}} \\
    \midrule
...
\subsection{Synthesis: the missing joint artifact}
\label{subsec:synthesis_gap}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[font=\small]
    % axes
    \draw[->, thick] (0,0) -- (10,0) node[below right] {Learning-aware evidence};
    \draw[->, thick] (0,0) -- (0,6) node[above left, rotate=90] {Runtime monitored contract};
    \draw[dashed, gray] (5,0) -- (5,6);
    \draw[dashed, gray] (0,3) -- (10,3);
    \node[below] at (2.5,0) {\scriptsize timing-only / weak learning signals};
    \node[below] at (7.5,0) {\scriptsize uncertainty/drift + quality signals};
    \node[left]  at (0,1.5) {\scriptsize static specs};
    \node[left]  at (0,4.5) {\scriptsize runtime monitoring};

    % points
    \node[draw, rounded corners=3pt, fill=gray!10] at (2.2,4.5) {\scriptsize AUTOSAR TIMEX};
    \node[draw, rounded corners=3pt, fill=gray!10] at (2.5,3.6) {\scriptsize ROS 2 Lifecycle};
    \node[draw, rounded corners=3pt, fill=gray!10] at (2.0,4.0) {\scriptsize ROSRV};
    \node[draw, rounded corners=3pt, fill=gray!10] at (3.2,4.2) {\scriptsize ROS-Monitoring};
    \node[draw, rounded corners=3pt, fill=blue!10, thick] at (7.4,4.5) {\scriptsize TIC + CAAI};
  \end{tikzpicture}%
  }
  \caption{A minimal taxonomy of contract approaches by (i) whether the artifact is designed for runtime monitoring and (ii) whether it is learning-aware (uncertainty/drift/quality evidence). TIC positions itself as a joint, runtime-auditable contract spanning both axes.}
  \label{fig:taxonomy}
\end{figure}
...
\begin{table}[htbp]
  \centering
  \caption{Example latency budget for a 50\,Hz low-level control loop
...
\[
  ...
  = \min\!\Bigl(S_s,\; w_1 S_t + w_2 S_q + w_4 S_o\Bigr) - w_5 P_v,
  \quad\text{subject to } S_s = 1.0
\]

This conjunctive form encodes a safety-engineering first principle: \emph{non-safety terms must not compensate for a safety violation}. If safety compliance drops below 1.0 (fallback triggered or safety invariant violated), the score collapses regardless of timing, quality, or observability. The weights $w_1,w_2,w_4,w_5$ are deployment-specific and must be calibrated through hazard analysis (FMEA/HAZOP) consistent with the applicable safety standard (ISO~26262, IEC~61508, or ISO~10218).

\vspace{4pt}
\noindent\fbox{\parbox{0.98\linewidth}{\footnotesize\textbf{NOTE:} The weights $w_1\!=\!0.15$, $w_2\!=\!0.15$, $w_3\!=\!0.35$, $w_4\!=\!0.10$, $w_5\!=\!0.25$ below demonstrate arithmetic only. Production deployments \emph{must} derive $w_i$ from system-level hazard analysis (FMEA or HAZOP) against the applicable safety standard (ISO~26262, IEC~61508, or ISO~10218).}}
\vspace{4pt}

\begin{table}[htbp]
  \centering
  \caption{Worked example of RTI-Score computation for an AMR perception module over a 60-cycle monitoring window.}
  ...
  \caption*{\footnotesize With $w_1\!=\!0.15$, $w_2\!=\!0.15$, $w_3\!=\!0.35$,
    $w_4\!=\!0.10$, $w_5\!=\!0.25$: RTI-Score $= \min(1.0,\,0.138+0.131+0.078) - 0.013 = 0.334$.}
\end{table}
...
\subsubsection{Worked FMEA grounding for RTI-Score weights}
...
\begin{table}[htbp]
  \centering
  \caption{Example FMEA for a warehouse AMR perception path and mapping to TIC clauses.}
  \label{tab:fmea}
  \rowcolors{2}{tablealt}{white}
  \renewcommand{\arraystretch}{1.15}
  \setlength{\tabcolsep}{3pt}
...
versioned contract document (JSON, schema version \texttt{1.0.0}) and a runtime
``contract status'' stream. The document defines static intent (thresholds, deadlines,
ownership, and immutable \texttt{component.build} provenance); the status stream provides
the time-indexed evidence used for compliance. A complete JSON Schema (draft-07) is
maintained as \path{tic\_monitor/schema/tic.schema.json}.

\begin{table}[htbp]
  \centering
  \small
  \caption{Compact TIC contract schema excerpt (required fields).}
  ...
  \begin{tabular}{@{}L{0.30\linewidth}L{0.18\linewidth}L{0.42\linewidth}@{}}
    ...
    \midrule
    tic\_version & string & Contract schema version (semver, e.g., 1.0.0). \\
    component.name & string & Component identifier (node/service). \\
    component.build.type & string & Provenance kind (\texttt{git}, \texttt{container}, \texttt{semver}). \\
    component.build.id & string & Immutable identifier (commit hash, digest, or release tag). \\
    component.build.image & string & Optional OCI image reference when \texttt{type=container}. \\
...
\begin{designbox}{Minimal TIC contract example (JSON)}
\begin{verbatim}
{
  "tic_version": "1.0.0",
  "component": {
...
\subsubsection{Distinction from anytime algorithms and cascades}
\label{subsec:caai_novelty}
Anytime algorithms, cascades, and early-exit policies treat adaptation primarily as a \emph{compute} decision: given a time or energy budget, they select an inference depth or model to maximize expected accuracy~\cite{zilberstein1996anytime,deanboddy1988,shihliu1995}. In that framing, tier switching is an optimization knob and the safety controller is typically external (or implicit) to the inference policy.

CAAI's central difference is cross-layer: it couples tier selection to an explicit \emph{safety envelope} transition declared in TIC. The supervisor's invariant is not ``run the best model you can,'' but ``the active tier implies an envelope that keeps the system safe under worst-case timing and evidence conditions.'' Concretely, Tier~3 is defined to be safe-stop-compatible (Section~\ref{sec:caai_policy}) and therefore forces a conservative control mode (e.g., reduced speed cap and increased stopping distance) when uncertainty/drift rise or timing margin collapses. This makes tier-down a safety action, not only a compute action, and it yields an auditable mechanism: the inference policy and the physical operating envelope move together as a single contract-governed transition.

Shielding and runtime monitoring are the closest prior patterns~\cite{alshiekh2018}: a shield enforces hard constraints regardless of what a learned policy proposes. CAAI complements that by addressing \emph{which} learned component to run under varying timing/evidence conditions and by making the switching logic itself a declared, monitored contract artifact rather than an ad-hoc heuristic.
...
% --- DES-derived evaluation figures (Figs.~7--9) ---
\begin{figure}[htbp]
  \centering
...
Relative to the timing monitors, middleware practices, and learning-deployment patterns surveyed in Section~\ref{sec:lit}, this paper adds three artifacts that are specified jointly rather than as disconnected guidelines:
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  \item \textbf{TIC} (Sections~\ref{sec:tic}--\ref{sec:tic_schema}): a versioned, machine-readable contract co-specifying latency, uncertainty/drift, fallback, observability, and scalability obligations with required runtime evidence.
  \item \textbf{CAAI} (Section~\ref{sec:caai}): a deterministic tier supervisor whose downgrade/recovery rules are driven by TIC monitor signals and coupled to safety envelopes.
  \item \textbf{Adoption package} (Section~\ref{sec:eval}): a reporting protocol, discrete-event instantiation with statistical analysis (Section~\ref{subsec:sim_results}), and a reference monitor/schema for independent implementation.
\end{itemize}
The survey synthesis and case patterns (Sections~\ref{sec:lit}--\ref{sec:cases}) contextualize these artifacts; they are not separate empirical claims.
...
This paper's contribution is a contract specification and reference architecture, not an empirical benchmark. The DES in Section~\ref{subsec:sim_results} demonstrates the mechanics of TIC-governed tier switching under a validated queueing model; hardware validation of specific platforms is intentionally left to adopters using the reporting protocol in Section~\ref{sec:eval}. Parameters such as TIC clause thresholds, RTI-Score weights, and CAAI switching constants are design-time declarations that must be derived from system-level hazard analysis and calibrated on the target platform. Drift and uncertainty signals are proxies whose operational reliability depends on the chosen estimators and the quality of instrumentation. The paper does not claim to solve worst-case execution-time bounding for modern deep networks; instead it supplies architectural patterns and runtime monitoring hooks that make timing risk explicit and manageable in practice.
...
\ifthenelse{\equal{\blindmode}{true}}{%
  % Declarations are on the separate title page (title_page.tex) for double-blind review.
}{%
